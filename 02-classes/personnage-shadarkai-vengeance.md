# Personnage: Paladin Shadar-kai Vengeance

## Informations de base

| Élément | Valeur |
|---------|--------|
| **Nom** | Kael'thar (surnom: Kael) |
| **Race** | Shadar-kai (Elfe du Shadowfell) |
| **Classe** | Paladin niveau 3 |
| **Serment** | Vengeance |
| **Historique** | Haunted One |
| **Style de combat** | Dueling |
| **Alignement** | À définir |

---

## Traits raciaux Shadar-kai (MotM 2022)

| Trait | Effet |
|-------|-------|
| **Bonus stats** | +2 / +1 (à placer après les rolls) |
| **Darkvision** | 60 pieds |
| **Fey Ancestry** | Avantage vs charme, immunité sommeil magique |
| **Trance** | 4h de repos au lieu de 8h |
| **Perception** | Maîtrise gratuite |
| **Blessing of the Raven Queen** | Téléport 30 pi (action bonus), PB fois/long repos. Résistance à TOUS les dégâts jusqu'au prochain tour. |
| **Necrotic Resistance** | Résistance aux dégâts nécrotiques |

---

## Compétences maîtrisées

| Compétence | Source | Stat |
|------------|--------|------|
| Intimidation | Paladin | CHA |
| Athlétisme | Paladin | FOR |
| Survie | Haunted One | SAG |
| Religion | Haunted One | INT |
| Perception | Shadar-kai (gratuit) | SAG |

---

## Langue

- **Commun** (standard)
- **Elfique** (Shadar-kai)
- **Orc** (Haunted One)

---

## Équipement

| Slot | Équipement | Effet |
|------|------------|-------|
| Arme principale | **Weapon of Warning** (épée longue) | 1d8 + 6 (FOR +4, Dueling +2), Avantage Initiative groupe |
| Bras gauche | Bouclier (broche de Varek incrustée) | +2 CA |
| Armure | Cotte de mailles | CA 16 |
| À distance | 5 javelots | 1d6 + FOR, portée 30/120 |
| Focaliseur | **Broche de Varek** (incrustée au bouclier) | Corbeau noir stylisé, focus divin pour sorts |
| Pack | Pack d'explorateur | Équipement d'aventurier |
| Objet fétiche | **Pendentif de Kira** (porté au cou) | Cristal d'ombre, objet macabre Haunted One |

**CA totale: 18** (16 armure + 2 bouclier)

**HP: 31** (10 + 6 + 6 + CON×3)

---

## Objet magique

### Weapon of Warning - "Éveil de Kira" (Uncommon)

| Propriété | Effet |
|-----------|-------|
| **Type** | Épée longue |
| **Attunement** | Oui (1 slot utilisé) |
| **Dégâts** | 1d8 + 6 (normal) |
| **Avantage Initiative** | Toi **+ alliés dans 30 pi** |
| **Réveil automatique** | Alliés se réveillent si combat pendant repos |

**Effet:** Tant que l'arme est à portée et que tu es harmonisé, toi et tes alliés dans 30 pieds avez avantage sur les jets d'initiative.

#### Apparence

![Éveil de Kira](../images/eveil-de-kira.png)

Épée longue aux runes rougeoyantes gravées sur la lame. Une gemme écarlate orne le pommeau, pulsant faiblement quand le danger approche. Des volutes d'énergie cramoisie semblent danser autour de la lame dans l'obscurité.

#### Histoire de l'arme

Au cœur d'un ancien sanctuaire oublié, Kael'thar monta la garde toute une nuit auprès d'un autel brisé, jurant de protéger les voyageurs qui s'y réfugiaient. À l'aube, alors que le silence semblait total, la lame posée sur l'autel vibra soudainement, murmurant un avertissement divin.

Grâce à ce frisson prémonitoire, Kael'thar repoussa une embuscade de créatures tapies dans l'ombre. Reconnaissant sa vigilance et sa foi, l'esprit gardien du sanctuaire lia l'arme à son serment.

Depuis ce jour, la lame veille avec lui, l'avertissant toujours du danger avant qu'il ne frappe.

#### Le lien avec Kira

Quand le danger approche pendant le sommeil, ce n'est pas la lame qui réveille Kael'thar et ses alliés — c'est **l'esprit de Kira**, sa sœur défunte. Sa voix murmure doucement à l'oreille de ceux qu'elle protège, les tirant du sommeil juste avant que le danger ne frappe.

> *"Lève-toi, petit frère. Ils arrivent."*

---

## Stats

| Stat | Base | Race | 1-pour-1 | **Final** | **Mod** |
|------|------|------|----------|-----------|---------|
| **FOR** | 15 | +2 | +1 | **18** | **+4** |
| **CHA** | 15 | +1 | - | **16** | **+3** |
| **CON** | 15 | - | +1 | **16** | **+3** |
| **SAG** | 14 | - | - | **14** | **+2** |
| **DEX** | 13 | - | - | **13** | **+1** |
| **INT** | 11 | - | -2 | **9** | **-1** |

**Méthode utilisée:** 4d6 drop lowest, reroll les 1, puis +2/+1 race, puis 2× 1-pour-1

---

## Capacités de classe (Niveau 3)

### Paladin de base

| Capacité | Effet |
|----------|-------|
| **Divine Sense** | Détecte morts-vivants/fiélons (1 + CHA mod)/long repos |
| **Lay on Hands** | 15 HP de soins/long repos |
| **Divine Smite** | +2d8 radiant (slot niv 1), +1d8 vs morts-vivants/fiélons |
| **Divine Health** | Immunité aux maladies |
| **Dueling** | +2 dégâts avec arme à une main |

### Serment de Vengeance

| Capacité | Effet |
|----------|-------|
| **Abjure Enemy** | 1 créature effrayée + vitesse 0. Morts-vivants/fiélons = désavantage sur le save. |
| **Vow of Enmity** | Avantage sur TOUTES tes attaques contre 1 cible, 1 minute. Action bonus. |

---

## Sorts

### Slots: 3 (niveau 1)

### Sorts de Serment (toujours préparés, gratuits)

| Sort | Effet |
|------|-------|
| **Bane** | 3 cibles, -1d4 à leurs attaques et saves |
| **Hunter's Mark** | +1d6 dégâts/attaque, avantage pour traquer |

### Sorts préparés recommandés (4 sorts = CHA +3, niveau 3 ÷ 2 = 1)

| Sort | Rôle |
|------|------|
| **Wrathful Smite** | Dégâts + cible effrayée |
| **Shield of Faith** | +2 CA (action bonus) |
| **Cure Wounds** | Heal d'urgence |
| **Divine Favor** | +1d4 radiant/attaque (si CHA 16+) |

---

## Ressources (Niveau 3)

| Ressource | Quantité | Récupération |
|-----------|----------|--------------|
| Slots de sorts (niv.1) | 3 | Long repos |
| Lay on Hands | 15 HP | Long repos |
| Divine Sense | 4 (1 + CHA) | Long repos |
| Channel Divinity | 1 | Court ou long repos |
| Blessing of the Raven Queen | PB (2) | Long repos |
| Dés de vie | 3d10 | Long repos (moitié) |

---

## Backstory

### L'attaque du village

Quand il était jeune, le village de Kael'thar, dans le Shadowfell, a été attaqué par un seigneur vampire. Tous ceux qu'il connaissait ont été massacrés. Sa sœur **Kira** l'a protégé en le cachant dans une cache derrière un mur, et il l'a vue se faire égorger à travers une fente. Avant de quitter le village, Kael'thar a pris le pendentif qu'elle portait et le garde depuis toujours.

### Résumé

Kael'thar était un jeune enfant d'un clan Shadar-kai du Shadowfell. Il a survécu au massacre grâce au sacrifice de sa sœur **Kira**.

Recueilli par **Varek**, un vieux Paladin Shadar-kai exilé qui servait la Raven Queen, il a été entraîné pendant 15 ans. Varek lui a appris que la mort est naturelle - mais que ceux qui la refusent (morts-vivants) ou la corrompent (nécromanciens) doivent être chassés.

Il y a 5 ans, Kael'thar est rentré d'une mission pour trouver Varek torturé et tué. Près du corps: un symbole qu'il reconnaît - le même que sur l'armure du vampire qui a tué sa sœur.

Il a juré vengeance. Il traque maintenant ce vampire à travers les plans.

### Personnages clés

| Qui | Relation | Statut |
|-----|----------|--------|
| **Kira** | Sœur | Morte (égorgée par un vampire) |
| **Varek** | Mentor | Mort (torturé, assassiné) |
| **Le Seigneur Vampire** | Antagoniste | En vie (traqué) |
| **Le clan** | Famille | Massacré |

### Objets importants

| Objet | Signification |
|-------|---------------|
| **Pendentif de Kira** | Cristal d'ombre qu'elle portait. Froid au toucher. Kael'thar le porte toujours autour du cou — c'est son objet fétiche (Haunted One). |
| **Broche de Varek** | Corbeau noir stylisé, symbole sacré de la Raven Queen. Varek l'a léguée à Kael'thar avant sa mort. Kael l'a fait incruster sur son bouclier et s'en sert comme focus divin pour lancer ses sorts de paladin. |
| **Symbole du vampire** | Trouvé près du corps de Varek. Sa seule piste. |

### Mystères non résolus (hooks DM)

- Qui est ce **Seigneur Vampire**?
- Pourquoi a-t-il attaqué le clan il y a 20 ans?
- Pourquoi est-il revenu tuer Varek?
- Qu'est-ce que Varek savait qu'il n'a jamais révélé?
- Le vampire cherche-t-il aussi Kael'thar?

---

## Personnalité

### Traits

| Trait | Description |
|-------|-------------|
| **Silencieux** | Parle peu. Quand il parle, c'est important. |
| **Observateur** | Regarde. Analyse. Identifie les menaces. |
| **Implacable** | Une fois sa cible choisie, rien ne l'arrête. |
| **Détaché** | Les émotions sont un luxe. La mission prime. |
| **Loyal** | Ceux qui gagnent sa confiance ont un allié pour la vie. |

### Défaut

Colère froide et calculée quand on menace les siens ou quand il trouve une piste vers le vampire.

### Trait de background: Heart of Darkness

Les gens ordinaires reconnaissent que tu as vécu quelque chose de terrible. Ils t'aident, toi et tes compagnons, en offrant abri, nourriture et assistance — tant que tu ne représentes pas un danger évident pour eux.

### Phrases typiques

- *"..."* (silence)
- "Celui-là. C'est lui le danger."
- "Reste derrière moi."
- "La mort vient pour tous. J'accélère juste le processus pour certains."
- "Tu as fait une erreur."

---

## Tactiques de combat

### Round 1 (engagement)

1. **Blessing of the Raven Queen** → Téléporte vers la menace principale
2. **Vow of Enmity** (action bonus) → Avantage sur toutes tes attaques
3. **Attaque** avec avantage → Smite sur crit

### Rounds suivants

- Martèle la cible avec avantage
- Smite sur les crits (dégâts doublés)
- Si cible fuit → Misty Step ou Blessing pour rattraper

### Quand Smite

| Situation | Smite? |
|-----------|--------|
| Crit (nat 20) | **TOUJOURS** |
| Boss/menace principale | Oui |
| Mort-vivant/fiélon | Oui (+1d8 bonus) |
| Ennemi faible | Non, garde tes slots |

---

## Niveau 4 (futur)

**Choix:** ASI ou Don

| Option | Effet |
|--------|-------|
| **+2 FOR** | FOR 18 → **20 (+5)** — Maximum! +7 attaque, +7 dégâts |
| Sentinel | Contrôle de zone, réaction pour frapper |
| +1 FOR / +1 CON | FOR 19, CON 17 (moins optimal) |

**Recommandation:** +2 FOR pour atteindre 20 FOR au niveau 4.

---

## Notes de campagne

- **Type:** Horreur (zombies, squelettes, vampires, etc.)
- **Autre joueur:** Rogue Shadar-kai Phantom (possible lien backstory)
- **DM:** Permet reroll des 1 sur les stats
